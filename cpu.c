...
#define DP (reg[0x8E]|((uint16_t)reg[0x8F]<<8))
static uint8_t reg4rRaw(uint16_t i, uint8_t *d) {
	if((i & 0x0F0) == 0x0F0) return *d = reg[0x78] & 0xF;
...
static void reg4wRaw(uint16_t i, uint8_t x) {
	...
	if(i == 0x109) { // TODO: FIX
		x |= onKey << 2;
		if((x & 8) != (Isel & 8)) {
			uint8_t temp = ((reg[0x102>>1] & 1) << 4) | (reg[0x101>>1] >> 4);
			reg[0x101>>1] &= 0x0F; reg[0x102>>1] &= 0xF0;
			reg[0x101>>1] |= reg[0x79] << 4;
			reg[0x102>>1] |= reg[0x79] >> 4;
			reg[0x79] = temp;
			Isel = x;
		}
	}
...
#endif
#endif
	if((i & 0x0F0) == 0x0F0) reg[0x78] = (reg[0x78] & 0xF0) | (x & 0xF);
	...
	else if(i == 0x11A) reg[0x7A] = (reg[0x7A] & 0xF0) | (x & 0xF);
	else if(i == 0x11B) reg[0x7A] = (reg[0x7A] & 0x0F) | (x << 4);
...
uint8_t reg8r(uint16_t i) {
	if((i & 0x0F0) == 0x0F0) {
		lastLow = reg[0x78] & 0xF; lastHigh = reg[0x78] >> 4;
		return reg[0x78];
	}
...
// TODO: FIX USING (1FX) AS (I) WHILE NOT MESSING OTHER STUFF UP
#define WYX ((op & 0x2FF) == 0x2FF ? I : add4(((op >> 1) & 0x100) | YX, offset))
#define WYXS ((op & 0x2FF) == 0x2FF ? I : add4(((op >> 1) & 0x100) | YX,OFFSET))
#define CHECK_WYXS_REP do{if(rep && (op & 0x2FF) == 0x2FF) Iadd(2 - S);}while(0)
	...
	for(int repeat = rep, offset = 0; repeat >= 0; repeat--, offset++) {
		...
		if(0) {
			...
			printf("A=%02X I=%03X (I)=%02X DP=%04X SP=%X\n",
				reg[0x78], I, reg8r(I), DP, reg[0x118>>1] & 0xF);
		}
		uint16_t op = word(PC++);
		...
		if(op == 0xB000) {debugWindow(); debug("Break instruction"); stopped = 1;}
			...
			if((op & 0x18) == 0x18)
				reg8w(0x0FF, reg8r(0x0FF) >> (op & 4 && DPsave < 0x4000 ? 2 : 1));
			if(op & 0x20) {
			...
			// TODO: CHECK lastLow/lastHigh
			uint8_t temp = regSr(I); regSw(I, regSr(JYX)); regSw(JYX, temp);
			if(rep) Iadd(2 - S);
		}
		else if(op < 0x0C00) {
		...
		else if(op < 0x1400) {regSw(0x0FF, regSr(WYXS)); CHECK_WYXS_REP;}
		else if(op < 0x1800) {
			// TODO: CHECK lastLow/lastHigh
			uint8_t temp = regSr(WYXS);
			regSw(WYXS, regSr(0x0FF));
			regSw(0x0FF, temp);
			CHECK_WYXS_REP;
		}
		else if(op < 0x1C00) {regSw(WYXS, regSr(0x0FF)); CHECK_WYXS_REP;}
		else if(op < 0x1E00) regSw(0x0FF, op);
		...
		else if(op < 0x8000) COND(regSr(0x0FF), op & SMASK);
		else if(op < 0xA000) {
			...
			if(repeat);
			else if(((reg4r(WYX) >> B) & 1) == ((op >> 11) & 1)) C_JUMP_CALL;
		...
		else {regSw(0x0FF, ADD(SB, regSr(0x0FF), op & SMASK)); M_JUMP_CALL;}
		if(writeRepeat) {
